...
public class AVLTree {
    ...
    private void balance(Node node) {
        ...
        Boolean avl = true;
        do {
            if (node_middle == parent.getLeft() && (-1 <= parent.getBf() - 1 && parent.getBf() - 1 <= 1)) {
                parent.subBf();
                node_prev = node_middle;
                node_middle = parent;
                if (parent != null && parent.getBf() == 0) {
                    parent = null;
                } else {
                    parent = parent.getParent();
                }
            } else if (node_middle == parent.getRight() && (-1 <= parent.getBf() + 1 && parent.getBf() + 1 <= 1)) {
                parent.addBf();
                node_prev = node_middle;
                node_middle = parent;
                if (parent != null && parent.getBf() == 0) {
    ...
    private void printRow(int length, int blank, int row, List<Node> list) {
        ...
        if (row == 1) {
            ...
            for (Node t : list) {
                if (t.getLeft() != null) {
                    newlist.add(t.getLeft());
                    System.out.print(t.getLeft().getItem() + x.toString());
                } else if (t.getItem() != 0 || row <= length) {
                    newlist.add(new Node(0));
                    System.out.print("x " + x.toString());
                }
                if (t.getRight() != null) {
                    newlist.add(t.getRight());
                    System.out.print(t.getRight().getItem() + y.toString());
                } else if (t.getItem() != 0 || row <= length) {
                    newlist.add(new Node(0));
    ...
    private void LeftLeftRotate(Node node) {
        ...
        if (node.getBf() == -1) {//只有左节点时，parent转换后没有子节点
            parent.setBf(0);
            node.setBf(0);
        } else if (node.getBf() == 0) {//node有两个子节点，转换后parent有一个左节点
            parent.setBf(-1);
            node.setBf(1);
        }//node.getBf()为1时会调用左右旋转
    }
    ...
    private void RightRightRotate(Node node) {
        ...
        if (node.getBf() == 1) {
            node.setBf(0);
            parent.setBf(0);
        } else if (node.getBf() == 0) {//当node有两个节点时，转换后层数不会更改，左树比右树高1层，parent的右树比左树高一层
            parent.setBf(1);
            node.setBf(-1);
        }
    }
    ...
    private void LeftRightRotate(Node node) {
        ...
        if (!child.hasChild()) {
            node.setBf(0);
            parent.setBf(0);
        } else if (child.getBf() == -1) {
            node.setBf(0);
            parent.setBf(1);
        } else if (child.getBf() == 1) {
            node.setBf(-1);
            parent.setBf(0);
        } else if (child.getBf() == 0) {
            node.setBf(0);
            parent.setBf(0);
        }
        child.setBf(0);
    ...
    public void deleteNode(int item) {
        ...
        if (!node.hasChild()) {//叶子节点
            ...
            int tmp = last.getItem();
            deleteNode(last.getItem());
            node.setItem(tmp);
        }
        node = null;//GC
    }
    ...
    private void deleteAvl(Node node) {
        ...
        do {
            node_prev = node_middle;
            if (node_middle == parent.getLeft() && (parent.getBf() + 1 <= 1)) {
                if (parent.getBf() == 0) {
                    parent.addBf();
                    return;
                }
            ...
            } else if (node_middle == parent.getRight() && (parent.getBf() - 1 >= -1)) {
                if (parent.getBf() == 0) {
                    parent.subBf();
                    return;
                }
                ...
                Node child = middle.getBf() == -1 ? middle.getLeft() : (middle.getBf() == 1 ? middle.getRight() : (parent.getBf() == -1 ? middle.getLeft() : middle.getRight()));
